...
to represent a centralised electricity market.  The full optimal power flow
forumlation is available in \cite{pserc:mp_manual}.  The power system model is
described by defining the bus, branch and generator objects.  The power flow
...
flow formulation follows.  Since the formulation does not facilitate shutting
down expensive generators a unit-decommitment algorithm is provided.  Finally,
to provide an interface to agent participants that resembles that of real
electricity market, an auction wrapper around the optimal power flow is
described.
...
y_{ft}^l& =& \frac{y_s}{\tau e^{-j\theta}}\\
y_{tf}^l& =& \frac{-y_s}{\tau e^{\theta_{ph}}}\\
...
\subsubsection{AC OPF Formulation}
