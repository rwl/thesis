...
matrix $Y$ \cite[p.11]{pserc:mp_manual}.  The current injected at bus $i$ is
\begin{equation}
...
be put in series to form a single model used for all branches. The
transmission line has total series admittance $y_s = 1/(r_s+jx_s)$, where $r_s$
is the series resistance and $x_s$ is the series reactance, and total shunt
capacitance $b_c$. If the winding ratio is complex then $N = \tau
e^{j\theta_{shift}}$, where $\tau$ is the tap ratio and $\theta_{shift}$ is the
phase shift angle.
...
learning that was introduced by \citeA{sutton:1998}. TD methods do not
...
\citeA{petersScholar} provides a concise overview.
