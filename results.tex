...
\section{Simulation Results}
Rewards received by each agents are recorded for a simulated year of 364 trading
episodes.  To compare algorithms, the reward for each hour of the trading day is
averaged for each agent and plotted in the same figure.  Only results for agents
1 and 4 are given as agents 1 and 2 have identical portfolios and most of agent
3's portfolio consists of Hydro plant with zero cost.  The method of applying
percentage markups on marginal cost does not work for this type of plant and
almost identical results are found for all algorithms.
...
Figure \ref{fig:6_1} shows average rewards for each hour that compare the
Modified Roth-Erev method with the Stateful Roth-Erev method.  Also shown are
equivalent results for the Q-learning method.

\ifthenelse{\boolean{includefigures}}{
  \begin{figure}
    \centering
    \includegraphics{figures/fig6_2}
    \caption{Average rewards for two state configurations.}
    \label{fig:6_2}
  \end{figure}
}
...
reward received for pure demand forecast state and a combined forecast and
bus voltage profile state.  The bottom two plots show the same data for
Agent~4.
...
Figure \ref{fig:6_3} shows results from a repeat of the simulation with bus
voltage profile state, but with two offers required per generator.  Due to time
constraints and limited simulation resources only results for Q-learning and
ENAC are given.
...
that Q-learning is able to learn an effective policy that yields greater
profits that under passive behaviour for two different portfolios.  The
importance of utilising environment state data in a dynamic electricity setting
is illustrated by the differences in average reward received by the Modified
Roth-Erev method and the Stateful Roth-Erev method.  The optimal action for an
agent depends upon the current system load and Modified Roth-Erev is unable to
interpret this.  Stateful Roth-Erev is able to achieve roughly the same
performance as Q-learning.

Including bus voltage constraint data in the state for a discrete environment
would result in an impractical state space, but adding it a continuous
environment was straight-forward.  The results show that ENAC achieves greater
profits when presented with a combined demand forecast and bus voltage state
vector.  REINFORCE performs less well than ENAC, but also shows improvement
over the pure forecast case.  ENAC achieves equivalent, but not greater
performance than Q-learning in all periods of the trading day.  In robotics,
policy gradient methods are typically used with neural networks with more that
one input node and the effect of this network structure deserves
further investigation.

Simply changing the number of offers required to be submitted for each
generator from 1 to 2, increases the total number of discrete action
possibilities to 140,000.  Figure \ref{fig:6_3} shows that Q-learning is not
able to achieve the same level of reward as it is under the one offer case.
ENAC produces the same results under both configurations, but is not able to
use the increased flexibility in its offer structure to its advantage.
...
been shown to operate well with large state and action vectors.  Limitations of
the Modified Roth-Erev method have been demonstrated and Q-learning has been
outperformed by a policy gradient method when presented with a very large
action space.

AC optimal power flow adds enormously to simulation times when analysing entire
years of trading interactions.  The addition of bus voltage data to the state
vector improved the performance of the policy gradient methods, but it has not
been show if the same could not be achieve using perhaps on voltage angles from
