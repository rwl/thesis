...
the Roth-Erev method (described in Section \ref{sec:rotherev}) is applied to a
...
the public interest.  These papers show that previous research in this field by
the respected author was perceived to be of sufficient value for the approach
the be pursued further and a need to improve participant and market models was
recognised.  Despite the neglection of transmission system constraints, the
work is an ambitious attempt to extrapolate results out to consquences for a
national market.
...
paper is demonstrated a drawback of implementing custom learning methods that
...
important issue in certain electricity markets, but the learning algorithm and
network model are perhaps overly simple.  Agent-based simulation has the
potential to provide further insight into financial transmission rights and the
issue is one that ought to be revisited as advances in the field are made.
...
Recent agent-based simulation of electricity markets has been carried out
...
1MW at the slack bus and corresponding withdrawal of 1MW at node $i$ [ref].
...
creating discrete representations of primarly continuous environments.
...
illustrates how the search spaces for learning methods grow dramatically as
models are expanded.  This is a problem that policy gradient learning methods
seek to address.
...
The modified Roth-Erev method proposed in this paper is later used in several
other publications.
...
AMES is the first agent-based electricity market simulation program to be
released as open source, but while there are serveral publications on the
project there are scarce papers involving its application.  This shows how
niche the field is and the challenge faced if such projects are to benefits
from the collaboration of communities that leads to the success of open source
development.
...
An two-tier model involves firms with two associated agents
...
system from \citeA{yao:07,yao:08} is used to validate the results against those
...
\end{itemize}
Improvements in these areas have the to potential to provide major financial
benefits to societies.

There is a trend in the literature towards the use of more complex learning
methods for participant behavioural representation and increaseingly accurate
...
simulation, have been shown to work well in similar problems.
