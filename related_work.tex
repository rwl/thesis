...
models used in previously published research.  \citeA{weidlich:08} provide a
...
% this chapter describes the software project in the context of other open
% source Electric Power Engineering programs.
...
In \citeA{bower:2001} a detailed model of electricity trading in England and
...
electricity market simulation and \citeA{bower:2001} use a relatively simple
...
\citeA{bunn:03} as part of an inquiry by the Competition Commission into whether
...
\citeA{visud:thesis}.  The custom method is designed specifically for the power
...
developed in \citeA{auer:03} for application to the $n$-armed bandit problem
...
this work are laid in \citeA{krause:nash04} with a comparison of agent-based
...
of the comparison results is analysed in \citeA{krause:nash06}.
...
Convergence to a Nash equilibrium is also shown in \citeA{sistani:06}. Boltzmann
...
technique to compare congestion management schemes in \citeA{krause:cong}. The
...
% Loading branches to their flow limits results in non-uniform nodal marginal
% prices where a nodal marginal price is the increase in the total system cost
% (the value of the objective function) when generation at that node is
% increased by 1MW.  These prices are commonly used in electricity market
% analysis as they may be determined from the Lagrangian multipliers on the
% active power balance constraints in the optimal power flow formulation.
...
To improve selection of the $\epsilon$ parameter for exploratory action
...
The SA-Q-learning method is also used in \citeA{tellidou:tacit} by researchers
...
This work is similar to earlier research from other institutions
...
\subsection{Market Power}
In \citeA{nicolaisen:2001} an agent-based model of a wholesale electricity
...
Genetic algorithms were a popular option for participant strategy modelling in
early agent-based electricity market research
\cite{richter:1998,petrov:2000,lane:2000}. \citeA{nicolaisen:2001} compares
...
\citeA{tesfatsi:psce} AMES is used to investigate strategic capacity withholding
...
\citeA{cincotti:09} from the University of Genoa used the modified Roth-Erev
...
from \citeA{yao:07} and \citeA{yao:08} is used to validate the results against
...
from \citeA{roth:aer}.  The results show that the presence of a forward
...
on sample data.  In \citeA{moody:98} a recurrent reinforcement learning method
...
% \begin{equation} S_t = \frac{\mbox{Average}(r_t)}{\mbox{Standard
% Deviation}(r_t)} \end{equation}
...
Simulations are conducted using artificial price data, equivalent to one year of
...
\citeA{moody:98} is contrasted with value function based methods.  Results from
...
The authors recognise that their grid market model could be generalised to other
