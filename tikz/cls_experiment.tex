\begin{figure}
  ...
  \begin{tikzpicture}[thick]
    ...
    \node (Agent) [final, rectangle split, rectangle split parts=3,
    		       text width=5.0cm, node distance=5cm] {
      \textbf{LearningAgent}
    ...
    edge [->] node[near end,left,xshift=-3mm,text width=6mm] {agents 1..*}
    (Agent) edge [->] node[near end,right,xshift=2mm,text width=6mm] {tasks
    1..*} (Task);
