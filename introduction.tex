...
This thesis is a comparison of algorithms which learn to trade power.  In this
chapter the motivation for research into the trade of electric power is
explained, the problem under consideration is defined and the principle research
contributions are stated.
% A reading guide and
% outline of the following chapters is also provided.
...
claimants' willingness to pay for the good, provide a device for efficient
...
The value of electricity to society
...
players and have been shown to be capable of learning highly complex
strategies \cite{tesauro:gammon}.
...
generating company, load serving entity or a firm of traders, make use of
multi-dimensional data that is mostly continuous in nature.  Certain data, such
...
available action in a given state. (A more detailed definition is given in
Section \ref{sec:valuebased}.)  When used with look-up tables they are
...
its parameters.  (More detail policy gradient methods is provided in Section
\ref{sec:policygradient}.)  They do not suffer from many of the problems that
...
the learning methods need not be altered.  They have been successfully applied
...
\begin{itemize}
  ...
  \item Simulation results that show how policy gradient reinforcement learning
  methods converge more slowly than value function based methods when learning
  simple power trade policies.
  \item Simulation results that show how agents using policy gradient
...
optimal power flow problems can be used to model electricity markets and
defines the reinforcement learning methods used in subsequent
chapters.

In Chapter \ref{ch:related_work} the research in this thesis is described in
the context of previous work which is related in terms of application field and
methodology.  Agent based electricity market simulation publications are
discussed with particular emphasis on the reinforcement learning methods used.
Previous applications of policy gradient learning methods in other market
settings are also reviewed.  Finally, the contribution made by releasing the
source code developed for this thesis as open source is shown in a reiview of the main
open source electric power Engineering software tools.
...
defines the security constrained optimal power flow formulation and describes
the wrapper that is used to provide a simplified interface to participants.
The chapter defines how learning agents are associated with a portfolio of
generators and/or dispatchable loads and how the submission of offers and/or bids and the receipt of rewards is coordinated.  It also shows
how artificial neural networks, that are used for policy function
approxiamtion, are structured to handle environment state information and the
design of look-up tables used with value function based methods.
...
In Chapter \ref{sec:furtherwork} ideas for futher work are outlined.  A model
of the UK transmission system, constructed from data in the National Grid
seven year statement, is described and it  is explained how the model could be
used, along with the advances made in this thesis, to study issues pertinent to
the nation.  The chapter goes on to explain how the option to use security
constrained optimal power flow, instead of the linearised DC formulations found in the literature,
provides opportunities for more accurate and revealing research.  The
need to model interelationships between multiple markets, including gas
and emissions, is recognised and a method for achieving this using the existing
model if described.

The primary conclusions of this thesis are summarised in Chapter
