...
\label{ch:conclusion}
In this final chapter the conclusions that can be drawn from the
results that have been presented in this thesis are summarized and several
ideas for further development of the contributions that have been made are
given.
...
resources become increasingly scarce.  Electric energy requires a unique
market design, but radical architecture changes can not be experimented with on
real systems.
...
policy gradient methods have not been previously applied.  Policy gradient
methods that can use function approximation techniques to operate in continuous
state and action spaces and have been used successfully in robotic control
applications.
...
In a second simulation the same algorithms were compared in a complex dynamic
...
simple problems.
% It has been how even moderately complex electricity market
% simulations produce state and action spaces that are too large for value
% function based methods to explore.
% Policy gradient methods have been shown to
...
It also explains is how a model formulated using data from National Grid
...
approximation and back-propagation techniques could also have been investigated.
...
\citeA{riedmiller05nfq} has presented Neuro-Fitted Q-Iteration (NFQ)
...
\citeA{bhatnagar09}. Like ENAC, they use function
...
to study issues pertinent to the UK energy industry.
...
topic could become of increasing interest and importance.
...
the phase shift angles by learning agents could be a topic of interest in
...
The data on the UK gas network provided by the \citeA{ngtsys2010} is
...
markets and the associated state and action spaces for agents would become very
